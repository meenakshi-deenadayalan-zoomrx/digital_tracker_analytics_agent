...
### Email Sender QC Override
When project_id is known, also apply sender category overrides:
```sql
-- Replace: e.sender_category
-- With:    COALESCE(rqes.sender_category, e.sender_category)
-- Add:     LEFT JOIN report_qc_email_senders rqes ON rqes.email_id = e.id AND rqes.project_id = {project_id}
```
